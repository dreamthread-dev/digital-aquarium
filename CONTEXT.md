...
1. **#20** 線画 PNG 準備・二値化（素材がないと他が進まない）
2. **#13〜#15** Python サーバー（最初に動かすと他のテストが楽）
3. **#16〜#19** Web 塗り絵アプリ（サーバーと並行開発可）
4. **#1〜#2** Godot 初期設定・背景 Shader
5. **#3〜#5** Boids / Quadtree / 奥行き
6. **#6〜#7** 登場・消滅演出
7. **#8** WebSocket 接続（サーバーと疎通確認）
8. **#9〜#12** デフォルト魚・UI・音
9. **#21** 全体統合テスト
